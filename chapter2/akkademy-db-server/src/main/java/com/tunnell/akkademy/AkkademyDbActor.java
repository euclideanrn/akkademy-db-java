 ...
 */
public class AkkademyDbActor extends AbstractActor {
    private final LoggingAdapter log = Logging.getLogger(context().system(), this);
    private final Map<String, Object> map = new HashMap<>();

    private AkkademyDbActor() {
        receive(ReceiveBuilder
                ...
                .match(GetKeysRequest.class, message -> {
                    log.debug("Received [GetKeys] request: {}", message);

                    Map<String, Object> resultKeys = map.keySet().stream()
                            .collect(Collectors.toMap(Function.identity(), x -> ""));

                    tellSuccess(new BatchResponse(resultKeys));
                })
